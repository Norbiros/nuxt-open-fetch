...
Components defined in your OpenAPI document under `components.schemas` are also exported as top-level types. Their names use the `Schema` prefix, so a component named `User` is exported as `SchemaUser`:
